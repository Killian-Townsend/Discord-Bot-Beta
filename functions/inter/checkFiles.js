  ...
  check(self){
    ...
    try{
      ...
      if(missing.length > 0){
        ...
        return [1,out];
      }else{console.log("[SYS] All Files Exist".brightMagenta)}
      
      
      // Perform Hash  Check
      if(hashChk){
        console.log("[SYS] Checking Hashes".brightMagenta)
        for(let i=0;i<keys.length;i++){
          
          if(FaH[keys[i]] === ""){ continue; }
          
          let file = keys[i];
          let hash = FaH[file];
          
          let txt = fs.readFileSync(`${file}`, 'utf8').toString();
          let realHash = checkFiles.hashFunc(txt);
          let comp = hash.localeCompare(realHash);
        ...
        // Hash Finish
        if(hashFail.length > 0){
          out = ``;
          for(let i=0;i<hashFail.length;i++){
            out = `${out}\n${hashFail[i]}`;
          }
          return [2,out];
        }else{console.log(colors.brightMagenta("[SYS] All Hashes Good"))}
      }
      // Exit
      return [];
    }catch(e){
      console.log(colors.bold.brightRed(`[ERROR] checkFiles Error | ${e.name}:${e.message}`));
    }
  },
  ...
  rehash: function(self){
    ...
    try{
      ...
      for(let i=0;i<files.length;i++){
        ...
        let c = files[i];
        if(FaH[c]===""){continue;}
        let f = fs.readFileSync(c, 'utf8').toString();
        let h = checkFiles.hashFunc(f);
        FaH[c] = h;
      ...
      console.log(`[ERROR] rehash Error | ${e.name}  ${e.message}`.brightRed.bold);
      return;
    }
  },
  
  hashFunc: function(inp){
    var hexcase=0;
    let d = hex_md5(inp);
    return d;
